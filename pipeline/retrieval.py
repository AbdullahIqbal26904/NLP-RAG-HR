...
class UrduHybridRetriever:
    """Hybrid BM25 + Semantic retriever for Urdu data with multilingual embeddings."""
    ...
    def __init__(self):
        ...
        pc = Pinecone(api_key=os.getenv("PINECONE_API_KEY"))
        self.candidate_index = pc.Index(
            os.getenv("PINECONE_URDU_CANDIDATE_INDEX", "urdu-candidates")
        )
        self.job_index = pc.Index(
            os.getenv("PINECONE_URDU_JOB_INDEX", "urdu-jobs")
        )

        resumes = _load_resumes_urdu()
        jobs = _load_jobs_urdu()

        self._candidate_lookup = {str(r["id"]): r for r in resumes}
        self._job_lookup = {str(j["job_id"]): j for j in jobs}

        self._candidate_ids = [str(r["id"]) for r in resumes]
        self._job_ids = [str(j["job_id"]) for j in jobs]
        self._candidate_texts = [serialize_candidate_urdu(r) for r in resumes]
        self._job_texts = [serialize_job_urdu(j) for j in jobs]

        self._bm25_candidates = BM25Okapi([t.split() for t in self._candidate_texts])
        self._bm25_jobs = (
            BM25Okapi([t.split() for t in self._job_texts])
            if self._job_texts
            else None
        )
    ...
    def retrieve(
        self, query: str, mode: SearchMode, strategy: str = "hybrid_rrf_ce"
    ) -> list[dict]:
        ...
        if strategy == "semantic_only":
            ranked = sorted(semantic, key=lambda x: x.get("score", 0.0), reverse=True)[
                : self.top_k_final
            ]
            for i, doc in enumerate(ranked, 1):
                doc.setdefault("semantic_score", doc.get("score", 0.0))
    ...
    def _enrich_results(self, results: list[dict], mode: SearchMode) -> list[dict]:
        lookup = (
            self._candidate_lookup if mode == "candidate" else self._job_lookup
        )
        for r in results:
            original = lookup.get(r["id"])
            if original:
    ...
    def _semantic_search(self, query_vector: list, mode: SearchMode) -> list[dict]:
        ...
        response = index.query(
            vector=query_vector, top_k=self.top_k, include_metadata=True
        )
        return [
            {
                "id": m["id"],
    ...
    def _cross_encoder_rerank(self, query: str, docs: list[dict]) -> list[dict]:
        ...
        for doc, score in zip(docs, scores):
            doc["cross_encoder_score"] = float(score)
        reranked = sorted(
            docs, key=lambda x: x["cross_encoder_score"], reverse=True
        )
        for rank, doc in enumerate(reranked):
            doc["final_rank"] = rank + 1
        return reranked
